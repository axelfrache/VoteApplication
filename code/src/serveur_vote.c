    ...
    void traitementLireElection(LireElectionCmd *cmd) {
        ...
        // Lecture directe de l'élection par identifiant
    ...
    void traitementModifierElection(ModifierElectionCmd *cmd) {
        ...
        int size = strlen(cmd->identifiant);
        if (!electionExists(db, cmd->identifiant, size)) {
            printf("L'élection avec l'identifiant '%s' n'existe pas.\n", cmd->identifiant);
        } else {
            //updateElectionByIdentifiant(db, cmd->identifiant, size, cmd->nouvelleQuestion);
